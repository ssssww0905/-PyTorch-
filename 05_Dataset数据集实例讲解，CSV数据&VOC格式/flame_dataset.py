...
class FlameDataset(Dataset):
    ...
    def __init__(self, dataset_path, transform=None):
        ...
        assert os.path.exists(self.image_path), "JPEGImages not exist"
        assert os.path.exists(self.target_path), "Annotations not exist"
        self.image_index_list = sorted([s.split(".")[0] for s in os.listdir(self.image_path)])
        self.target_index_list = sorted([s.split(".")[0] for s in os.listdir(self.target_path)])
        assert self.image_index_list == self.target_index_list, \
            "image name does not match target name"
...
def eg_1():
    ...
    flame_dataset = FlameDataset(dataset_path=dataset_path, transform=transform)
